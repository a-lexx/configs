...
add bridge=bridge1 client-to-client-forwarding=yes local-forwarding=no name=\
    datapath1
add bridge=guest_bridge client-to-client-forwarding=no local-forwarding=no \
    name=guest_datapath
/caps-man security
...
add authentication-types=wpa2-psk comment=2.4hz_password encryption=aes-ccm \
    name=easy
add authentication-types=wpa2-psk encryption=aes-ccm name=guest_wifi_security
...
add datapath=guest_datapath name=guest_wifi_cfg security=guest_wifi_security \
    ssid=1519_guests
/caps-man interface
...
add configuration=cfg_5G disabled=no l2mtu=1600 mac-address=2C:C8:1B:59:A6:19 \
    ...
    2CC81B59A619
add configuration=guest_wifi_cfg disabled=no mac-address=00:00:00:00:00:00 \
    master-interface=none name=cap17 radio-mac=00:00:00:00:00:00 radio-name=\
    ""
/interface list
add name=WAN
add name=LAN
...
add name=guest_dhcp_pool ranges=192.168.88.100-192.168.88.200
/ip dhcp-server
add address-pool=dhcp interface=bridge1 lease-time=10m name=dhcp1
add address-pool=guest_dhcp_pool interface=guest_bridge name=guest_dhcp
/queue simple
add max-limit=5M/5M name=guest_wifi_queue target=192.168.88.0/24
/caps-man manager
set enabled=yes
...
add address=192.168.88.50/24 interface=guest_bridge network=192.168.88.0
/ip dhcp-client
